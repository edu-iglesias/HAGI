    ...
    <div class="col-sm-6 col-lg-6 col-centered padding main">
        ...
        {{ Form::open(array('class' => 'form-signin registration-form', 'role' => 'form')) }}
            ...
            <div class="form-group @if ($errors->has('firstname')) has-error @endif" id="step1-name">
                    {{ Form::text('firstname',Session::get('firstname'), ['class'=>'form-control reg-form', 'placeholder' => 'First name','maxlength'=>'100']) }}
                ...
                <div class="form-group @if ($errors->has('lastname')) has-error @endif">
                    {{ Form::text('lastname',Session::get('lastname'), ['class'=>'form-control reg-form', 'placeholder' => 'Last name','maxlength'=>'100']) }}
                ...
                <div class="form-group @if ($errors->has('birthdate')) has-error @endif" id="datepicker" data-date="{{ date('Y-m-d') }}T" data-date-format="yyyy-mm-dd">
                    {{ Form::text('birthdate',Session::get('birthdate'), ['class'=>'form-control reg-form','maxlength'=>'255','id'=>'birthdate', 'placeholder' => 'Birth date', 'onchange'=>"checkDate(this.value)"]) }}
                ...
                <div class="form-group @if ($errors->has('contact')) has-error @endif">
                    {{ Form::text('contact',Session::get('contact'),array('class' => 'form-control reg-form', 'placeholder' => 'Contact number', 'id'=>'contact')) }}
                    @if ($errors->has('contact')) <p class="help-block">{{ $errors->first('contact') }}</p> @endif
                </div>

                {{ Form::submit('Next', ['id'=>'submit submit-reg','class' => 'btn btn-lg btn-success btn-block sbmt']) }}
